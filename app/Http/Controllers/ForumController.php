...
class ForumController extends Controller
{
    ...
    public function getForumReports(Request $request, $forumId)
    {
        ...
        // Check if user is forum creator, admin, or moderator
        $isCreator = $forum->created_by === $user->id;
        $member = $forum->members()->where('user_id', $user->id)->first();
        $userRole = $member ? $member->pivot->role : null;
        $isAdminOrModerator = in_array($userRole, ['admin', 'moderator']);

        if (!$isCreator && !$isAdminOrModerator) {
            return response()->json([
                'status' => 403,
                'message' => 'Only forum creator, admins, and moderators can view reports',
            ], 403);
        }
        ...
        // Note: reportable_type is stored as 'post' in the database
        $reports = Report::where('reportable_type', 'post')
        ...
        // Get post titles separately to avoid relationship issues
        $postTitles = ForumPost::whereIn('id', $postIds)
            ->pluck('title', 'id')
            ->toArray();
        
        $reports = $reports->map(function ($report) use ($postTitles) {
            return [
                'id' => $report->id,
                'post_id' => $report->reportable_id,
                'post_title' => $postTitles[$report->reportable_id] ?? 'Deleted Post',
