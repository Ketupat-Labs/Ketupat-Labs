            ...
            <form method="POST" action="{{ route('lessons.update', $lesson->id) }}" enctype="multipart/form-data"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" name="duration" id="duration" min="5"
                                class="block w-full border-gray-300 rounded-lg shadow-sm p-3 focus:border-blue-500 focus:ring focus:ring-blue-500/20 transition"
                                value="{{ old('duration', $lesson->duration) }}" placeholder="e.g., 45">
                        </div>
                    ...
                    <div>
                        ...
                        @if($lesson->material_path)
                            <div class="mb-2 text-sm text-gray-600">
                                Current file: <a href="{{ Storage::url($lesson->material_path) }}" target="_blank"
                                    class="text-blue-600 hover:underline">{{ basename($lesson->material_path) }}</a>
                            </div>
                        @endif
                        ...
                        <p class="mt-2 text-xs text-gray-500">Upload a new file to replace the current one (optional)
                            (Max 25MB)</p>
                    </div>
                </div>
                ...
                <div class="relative">
                    ...
                    <div class="relative flex justify-center">
                        <span class="px-3 bg-gray-50 text-sm text-gray-500">Lesson Content Below</span>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div class="border-l-4 border-green-500 pl-4">
                        ...
                        <p class="text-sm text-gray-600 mt-1">Build your lesson using blocks - add text, headings,
                            images, videos, and interactive games</p>
                    </div>

                    <!-- React Block Editor Container -->
                    <div id="block-editor-root"
                        data-initial-blocks="{{ json_encode($lesson->content_blocks['blocks'] ?? []) }}"></div>
