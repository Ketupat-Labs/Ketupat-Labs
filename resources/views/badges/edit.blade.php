<x-app-layout>
                ...
                <form action="{{ route('badges.update', $badge->id) }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="3" required
                                  class="shadow appearance-none border rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('description') border-red-500 @enderror">{{ old('description', $badge->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="icon" id="icon" value="{{ old('icon', $badge->icon) }}" required maxlength="10"
                        ...
                        <div class="mt-3">
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach(['🏆', '⭐', '🎯', '🎮', '💎', '🥇', '🥈', '🥉', '👑', '🔥', '💪', '🎓', '📚', '✨', '🌟', '💡', '🚀', '🎨', '🎵', '⚡'] as $emoji)
                                    <button type="button" onclick="document.getElementById('icon').value='{{ $emoji }}'; updatePreview();" 
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex items-center gap-4">
                            <input type="color" name="color" id="color" value="{{ old('color', $badge->color) }}" required
                                   class="h-12 w-20 border-2 border-gray-300 rounded cursor-pointer">
                            <input type="text" id="color-hex" value="{{ old('color', $badge->color) }}" readonly
                    ...
                    <div class="mb-6">
                        ...
                        <input type="number" name="xp_reward" id="xp_reward" value="{{ old('xp_reward', $badge->xp_reward) }}" 
                    ...
                    {{-- Preview --}}
                    <div class="mb-6 p-6 bg-gray-50 rounded-lg border-2" id="preview-box" style="border-color: {{ $badge->color }};">
                        <p class="text-sm text-gray-600 mb-3 font-semibold">Pratonton:</p>
                        <div class="flex items-center">
                            <span id="preview-icon" class="text-6xl mr-4">{{ $badge->icon }}</span>
                            <div>
                                <h3 id="preview-name" class="text-2xl font-bold" style="color: {{ $badge->color }};">{{ $badge->name }}</h3>
                                <p id="preview-desc" class="text-gray-600">{{ $badge->description }}</p>
                                <p class="text-sm text-gray-500 mt-1"><span id="preview-xp">{{ $badge->xp_reward }}</span> XP</p>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        function updatePreview() {
            document.getElementById('preview-icon').textContent = document.getElementById('icon').value;
            ...
            document.getElementById('preview-xp').textContent = document.getElementById('xp_reward').value;
        }
