                                        ...
                                        @forelse($assignments as $assignment)
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <button type="button" @click="openEdit({{ $assignment->lesson_id }}, '{{ addslashes($assignment->lesson->title) }}')" 
                                ...
                                <div class="border border-gray-200 rounded-md max-h-60 overflow-y-auto bg-gray-50 p-2 space-y-2">
                                    ...
                                    @foreach($classrooms as $classroom)
                                        <div class="flex items-center p-2 bg-white rounded-md border border-gray-200 hover:bg-indigo-50"
                                            ...
                                            <input id="edit_class_{{ $classroom->id }}" name="classroom_ids[]" type="checkbox" value="{{ $classroom->id }}"
                                                :checked="editClassroomIds.includes({{ $classroom->id }})"
                                                @change="toggleEditClass({{ $classroom->id }})"
                                                class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                                            <label for="edit_class_{{ $classroom->id }}" class="ml-2 block text-sm text-gray-900 cursor-pointer w-full">
                                                {{ $classroom->name }}
                                            </label>
                                        </div>
                                    @endforeach
