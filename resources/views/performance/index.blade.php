                    ...
                    <form method="GET" action="{{ route('performance.index') }}" id="filter_form"
                        ...
                        @if(auth()->user()->role === 'teacher')
                            <div class="flex flex-col gap-1">
                                ...
                                <select name="class_id" id="class_id" onchange="document.getElementById('hidden_filter_id').value='all'; this.form.submit();"
                        ...
                        <div class="flex flex-col gap-1 w-full md:w-auto" 
                             x-data="{ 
                                ...
                                selectedName: '{{ $selectedFilter === 'all' ? 'Semua Pelajaran & Aktiviti' : ($filterItems->firstWhere('id', $selectedFilter)['name'] ?? 'Sila Pilih Kelas Dahulu') }}',
                                items: {{ json_encode($filterItems->map(function($i){ return ['id' => $i['id'], 'name' => $i['name'] ]; })->prepend(['id' => 'all', 'name' => 'Semua Pelajaran & Aktiviti'])) }},
                                hasClass: {{ $selectedClass ? 'true' : 'false' }},
                                get filteredItems() {
                            ...
                            <label for="filter_dropdown" class="text-sm font-medium text-gray-700">Pelajaran / Aktiviti</label>
                            <input type="hidden" name="filter_id" id="hidden_filter_id" value="{{ $selectedFilter }}">
                            
                            <div class="relative min-w-[250px]">
                                <button type="button" 
                                    id="filter_dropdown"
                                    @click="if(hasClass) { open = !open; if(open) $nextTick(() => $refs.searchInput.focus()); }"
                                    :disabled="!hasClass"
                                    :class="hasClass ? 'cursor-default' : 'cursor-not-allowed opacity-50'"
                                    ...
                                    <span class="block truncate" x-text="hasClass ? selectedName : 'Sila Pilih Kelas Dahulu'"></span>
                                ...
                                <div x-show="open && hasClass" @click.away="open = false" 
                                ...
                                @if(!$selectedClass && auth()->user()->role === 'teacher')
                                <p class="mt-1 text-xs text-gray-500">Sila pilih kelas terlebih dahulu untuk melihat pelajaran dan aktiviti</p>
                                @endif
                            </div>
                        </div>
